...
/** Builds the shared globe tooltip HTML string. */
function tooltipHtml(name: string, rows: string): string {
  return `<div style="background:var(--bfc-base-2);color:var(--bfc-base-c);padding:10px 14px;border-radius:6px;border:1px solid var(--bfc-base-dimmed);pointer-events:none;min-width:160px">
    <div style="font-size:15px;font-weight:600;margin-bottom:8px">${name}</div>
    ${rows}
  </div>`;
}

function tooltipRow(swatch: string, label: string, value: string): string {
  return `<div style="display:flex;align-items:center;gap:8px;margin-top:4px">
    ${swatch}
    <span style="flex:1;color:var(--bfc-base-c-2);font-size:12px">${label}</span>
    <span style="color:var(--bfc-base-c);font-size:12px">${value}</span>
  </div>`;
}

function colorSwatch(color: string): string {
  return `<div style="width:22px;height:3px;background:${color};border-radius:2px;flex-shrink:0"></div>`;
}
...
export function DataUsageHeatmap() {
  ...
  const getLabel = useCallback(
    (feat: object) => {
      ...
      if (view === "data") {
        const usageRow = tooltipRow(
          colorSwatch(SUCCESS),
          "GB / user / month",
          `${data.usage}`,
        );
        return tooltipHtml(data.name, usageRow);
      }

      // Calls view — count in/out arcs per type
      ...
      const rows = TYPE_CONFIG
        ...
        .map(({ type, label, color }) => {
          ...
          return tooltipRow(colorSwatch(color), label, `↑&thinsp;${out}&emsp;↓&thinsp;${inc}`);
        })
        .join("");

      return tooltipHtml(data.name, rows);
    },
